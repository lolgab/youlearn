package it.lorenzogabriele.impara.server.jetty
...
class ApplicationServer(val port: Int, resourceBase: String, db: Db) {
  ...
  private val atmosphereHolder = {
    ...
    import it.lorenzogabriele.impara.server.rpc._
    ...
    val config = new DefaultAtmosphereServiceConfig[MainServerRPC](
      (clientId) =>
        new DefaultExposesServerRPC[MainServerRPC](
          new MainServerRPCImpl(db)(clientId)))
